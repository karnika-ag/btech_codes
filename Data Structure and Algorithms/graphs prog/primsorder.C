...
class Shortest
{
	...
	public:
		...
		void printtable()
		{
			...
			for(int p=0;p<i;p++)
			{
				cout<<"\n"<<b[p].vertex<<"  "<<b[p].known<<"  "<<b[p].dist<<"  "<<b[p].path<<"  "<<b[p].order<<"  ";
			}
		}
		int check()
		{
		...
		void findpath()
		{
			...
			while(check())
			{
				...
				{
					int g=get(t->vertex);
					if(t->length<b[g].dist&&b[g].known==0)
					{
						b[g].dist=t->length;
						//strcpy(t->vertex,b[g].path);
						for(int q=0;q<5;q++)
				...
				b[index].order=kk;
				kk++;
				cout<<"\n\nTable after "<<b[index].vertex<<" declared known.\n";
				printtable();
			}
			/*for(int y=0;y<i;y++)
			{
				if(strcmp(b[y].path,'\0')!=0)
					cout<<"  ("<<b[y].vertex<<","<<b[y].path<<") , ";
			}*/
		}
		void edges()
		{
			int o=2;
			cout<<"\nBy PRIM'S Alg Edges included are(in order):";
			while(o<=i)
			{
				for(int t=0;t<i;t++)
				{
