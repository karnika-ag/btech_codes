...
//server_addr.sin_addr.s_addr=INADDR_ANY;
 ...
 if(n1>=0) printf("connect succesful \n");
  ...
  int i=0,j=0,n;
  scanf("%d",&n);
  mpz_t Q[n],P,k,g,l,r,a,x1,x2,y1,y2,k1,M,session_key,C1,C2;
  mpz_init(P);
  mpz_init(k);
  mpz_init(g);
  mpz_init(l);
...
//step 3.. 
 ...
 if(n>0) gmp_printf("msg received : y2 is %Zd \n",y2);
 memset(msg,'\0',256);
 ...
 n=read(fd,msg,256);
 mpz_set_str(C1,msg,10);
 if(n>0) gmp_printf("msg received : C1 is %Zd \n",C1);
 memset(msg,'\0',256);
 n=read(fd,msg,256);
 mpz_set_str(C2,msg,10);
 if(n>0) gmp_printf("msg received : C2 is %Zd \n",C2);

 //decrypting 
 mpz_powm(session_key,C1,x1,P);
 gmp_printf("decrypted session key is : %Zd \n",session_key);
 mpz_invert(M,session_key,P);
 mpz_mul(M,M,C2);
 mpz_mod(M,M,P);
 gmp_printf("decrypted M is : %Zd \n",M);
